...
subroutine d3_end
 ...
 !aa67    d1 = 0.5_WP*(sigma(nsigma-1) - sigma(nsigma))
 !aa67    u_din_sur = 0.05_WP*(windx(node)**2 + windy(node)**2)
 !aa67    snu_m = 0.5_WP*(snu(nsigma,node) + snu(nsigma-1,node))
 !aa67    yy1(2) = (0.4d-3*u_din_sur**3*dmean*d1)/(snu_m*0.76_WP)
