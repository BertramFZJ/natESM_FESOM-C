...
! exchange_nod3D(arr(nsigma-1,myDim_nod2D+eDim_nod2D)) WP
! exchange_nod3D_full(arr(nsigma,myDim_nod2D+eDim_nod2D)) WP
...
contains
  ...
  subroutine check_mpi_comm(rn, sn, r_mpitype, s_mpitype, rPE, sPE)
    ...
    integer, intent(in) :: sn, rn, r_mpitype(rn), s_mpitype(sn), rPE(rn), sPE(sn)
    integer  :: n, sdebug, rdebug, status(MPI_STATUS_SIZE), request

    DO n=1,rn    
       call MPI_TYPE_SIZE(r_mpitype(n), rdebug, MPIerr)
       CALL MPI_ISEND(rdebug, 1, MPI_INTEGER, rPE(n), 10, MPI_COMM_FESOM_C, request, MPIerr)
    END DO

    DO n=1, sn
       call MPI_RECV(sdebug, 1, MPI_INTEGER, sPE(n), 10, MPI_COMM_FESOM_C,    &
            status, MPIerr)
       ...
       if (sdebug /= rdebug) then
          ...
          call MPI_ABORT( MPI_COMM_FESOM_C, 1 )
       end if
    END DO
...
!SH  print *,'exchng_i_begin',mype,sn,rn
...
!!$#endif
     ...
     DO n=1,rn    
        ...
        call MPI_IRECV(nod_array2D, 1, r_mpitype_nod2D_i(n), com_nod2D%rPE(n), &
             com_nod2D%rPE(n), MPI_COMM_FESOM_C, com_nod2D%req(n), MPIerr) 
     END DO

     DO n=1, sn

        call MPI_ISEND(nod_array2D, 1, s_mpitype_nod2D_i(n), com_nod2D%sPE(n), &
             mype, MPI_COMM_FESOM_C, com_nod2D%req(rn+n), MPIerr)
     END DO
 ...
 if (npes > 1) then
  ...
  DO n=1,rn         
     call MPI_IRECV(nod1_array2D, 1, r_mpitype_nod2D(n), com_nod2D%rPE(n), &
               com_nod2D%rPE(n),      MPI_COMM_FESOM_C, com_nod2D%req(2*n-1), MPIerr) 
 
     call MPI_IRECV(nod2_array2D, 1, r_mpitype_nod2D(n), com_nod2D%rPE(n), &
               com_nod2D%rPE(n)+npes, MPI_COMM_FESOM_C, com_nod2D%req(2*n),   MPIerr) 
  END DO  
  DO n=1, sn
     call MPI_ISEND(nod1_array2D, 1, s_mpitype_nod2D(n), com_nod2D%sPE(n), &
                    mype,      MPI_COMM_FESOM_C, com_nod2D%req(2*rn+2*n-1), MPIerr)

     call MPI_ISEND(nod2_array2D, 1, s_mpitype_nod2D(n), com_nod2D%sPE(n), &
                    mype+npes, MPI_COMM_FESOM_C, com_nod2D%req(2*rn+2*n),   MPIerr)
  END DO
...
! General version of the communication routine for 2D nodal fields
 ...
 if (npes > 1) then
  ...
  DO n=1,rn         
     call MPI_IRECV(nod1_array2D, 1, r_mpitype_nod2D(n), com_nod2D%rPE(n), &
               com_nod2D%rPE(n),        MPI_COMM_FESOM_C, com_nod2D%req(3*n-2), MPIerr) 
 
     call MPI_IRECV(nod2_array2D, 1, r_mpitype_nod2D(n), com_nod2D%rPE(n), &
               com_nod2D%rPE(n)+npes,   MPI_COMM_FESOM_C, com_nod2D%req(3*n-1), MPIerr) 

     call MPI_IRECV(nod3_array2D, 1, r_mpitype_nod2D(n), com_nod2D%rPE(n), &
               com_nod2D%rPE(n)+2*npes, MPI_COMM_FESOM_C, com_nod2D%req(3*n),   MPIerr) 
  END DO  
  DO n=1, sn
     call MPI_ISEND(nod1_array2D, 1, s_mpitype_nod2D(n), com_nod2D%sPE(n), &
                    mype,        MPI_COMM_FESOM_C, com_nod2D%req(3*rn+3*n-2), MPIerr)

     call MPI_ISEND(nod2_array2D, 1, s_mpitype_nod2D(n), com_nod2D%sPE(n), &
                    mype+npes,   MPI_COMM_FESOM_C, com_nod2D%req(3*rn+3*n-1), MPIerr)

     call MPI_ISEND(nod3_array2D, 1, s_mpitype_nod2D(n), com_nod2D%sPE(n), &
                    mype+2*npes, MPI_COMM_FESOM_C, com_nod2D%req(3*rn+3*n),   MPIerr)
  END DO
...
! stored in (vertical, horizontal) format
 ...
 if (npes > 1) then
    ...
    if ((nl1<nsigma-1) .or. (nl1>nsigma)) then
       if (mype==0) then
...
! stored in (vertical, horizontal) format
 ...
 if (npes > 1) then
    ...
    if ((nl2<nsigma-1) .or. (nl2>nsigma)) then
       if (mype==0) then
...
if (npes>1) then
 ! nod_array3D(n_val,nl1,nod2D_size)
  ...
  if ((nl1<nsigma-1) .or. (nl1>nsigma) .or. (n_val > 3)) then
...
!!$ integer  :: nz, nh, nc
!!$ real(kind=WP) :: edge_array3D(nsigma-1,edge2D) 
!!$  
...
!!$ real(kind=WP), intent(inout)  :: edge_array2D(:)
!!$
!!$ integer  :: n, sn, rn
!!$
!!$ if (npes> 1) then
...
!!$               com_edge2D%rPE(n), MPI_COMM_FESOM_C, com_edge2D%req(n), MPIerr) 
!!$  END DO  
!!$  DO n=1, sn
!!$     call MPI_ISEND(edge_array2D, 1, s_mpitype_edge2D(n), com_edge2D%sPE(n), &
!!$                    mype, MPI_COMM_FESOM_C, com_edge2D%req(rn+n), MPIerr)
!!$  END DO
!!$  
...
!SH print *,'Exchange elem3D'
!SH print *,ubound(elem_array3D,1),mype,ubound(elem_array3D,2),myDim_elem2D+eDim_elem2D+eXDim_elem2D

if (npes> 1) then

   nl1=ubound(elem_array3D,1)

   if (ubound(elem_array3D,2)<=myDim_elem2D+eDim_elem2D) then !SH < instead of <=
      ...
      if (nl1==nsigma .or. nl1==nsigma-1) then
         ...
         DO n=1,rn         
            call MPI_IRECV(elem_array3D, 1, r_mpitype_elem3D_full(n,nl1,1), &
                 ...
                 com_elem2D_full%rPE(n), MPI_COMM_FESOM_C, &
...
! stored in (vertical, horizontal) format
 ...
 n_val = ubound(elem_array3D,1)
  ...
  if ((nl1<nsigma-1) .or. (nl1>nsigma) .or. (n_val > 4)) then
 ...
 if (ubound(elem_array3D,3)<=myDim_elem2D+eDim_elem2D+eXDim_elem2D) then
    ...
    elem_full_flag = .false.
     ...
     DO n=1,rn         
        call MPI_IRECV(elem_array3D, 1, r_mpitype_elem3D(n,nl1,n_val), com_elem2D%rPE(n), &
                       com_elem2D%rPE(n), MPI_COMM_FESOM_C, com_elem2D%req(n), MPIerr) 
     END DO
     DO n=1, sn
        call MPI_ISEND(elem_array3D, 1, s_mpitype_elem3D(n,nl1,n_val), com_elem2D%sPE(n), &
                       mype, MPI_COMM_FESOM_C, com_elem2D%req(rn+n), MPIerr)
     END DO
     ...
     DO n=1,rn         
        call MPI_IRECV(elem_array3D, 1, r_mpitype_elem3D_full(n,nl1,n_val), com_elem2D_full%rPE(n), &
                       com_elem2D_full%rPE(n), MPI_COMM_FESOM_C, com_elem2D_full%req(n), MPIerr) 
     END DO
     DO n=1, sn
        call MPI_ISEND(elem_array3D, 1, s_mpitype_elem3D_full(n,nl1,n_val), com_elem2D_full%sPE(n), &
                       mype, MPI_COMM_FESOM_C, com_elem2D_full%req(rn+n), MPIerr)
     END DO
...
!SH print *,'elem2D exchange'

 if (npes> 1) then

  if (ubound(elem_array2D,1)<=myDim_elem2D+eDim_elem2D) then !SH < instead of <=
...
IF ( mype == 0 ) THEN
    ...
    DO  n = 1, npes-1
       ...
       CALL MPI_SEND(sendbuf(1), nTS*nl1, MPI_DOUBLE_PRECISION, &
                   sender, 2, MPI_COMM_FESOM_C, MPIerr )

       DEALLOCATE(irecvbuf, sendbuf)
    ENDDO
ELSE
    CALL MPI_SEND( node_size, 1, MPI_INTEGER, 0, 0, MPI_COMM_FESOM_C, MPIerr )
...
IF ( mype == 0 ) THEN
    ...
    ENDDO
ELSE
    CALL MPI_SEND( elem_size, 1, MPI_INTEGER, 0, 0, MPI_COMM_FESOM_C, MPIerr )
...
IF ( mype == 0 ) THEN
   ...
   if (npes>1) then
      ...
      do  n = 1, npes-1
         ...
         call MPI_IRECV(recvbuf(1,start), n3D, MPI_DOUBLE_PRECISION, n, 2, MPI_COMM_FESOM_C, req(n), MPIerr)
      enddo
...
ELSE
   ...
   call MPI_SEND( arr3D, myDim_nod2D*nl1, MPI_DOUBLE_PRECISION, 0, 2, MPI_COMM_FESOM_C, MPIerr )
...
IF ( mype == 0 ) THEN
   ...
   if (npes>1) then
      ...
      do  n = 1, npes-1
         ...
         start = remPtr_nod2D(n)
         call MPI_IRECV(recvbuf(start), n2D, MPI_DOUBLE_PRECISION, n, 2, MPI_COMM_FESOM_C, req(n), MPIerr)
      enddo
...
ELSE
   ...
   call MPI_SEND( arr2D, myDim_nod2D, MPI_DOUBLE_PRECISION, 0, 2, MPI_COMM_FESOM_C, MPIerr )
...
! 
      ...
      do  n = 1, npes-1
         e3D = (remPtr_elem2D(n+1) - remPtr_elem2D(n))*nl1
         start = remPtr_elem2D(n)
         call MPI_IRECV(recvbuf(1,start), e3D, MPI_DOUBLE_PRECISION, n, 2, MPI_COMM_FESOM_C, req(n), MPIerr)
      enddo
...
ELSE
   ...
   call MPI_SEND( arr3D, myDim_elem2D*nl1, MPI_DOUBLE_PRECISION, 0, 2, MPI_COMM_FESOM_C, MPIerr )
...
IF ( mype == 0 ) THEN
   ...
   if (npes>1) then
      ...
      do  n = 1, npes-1
         ...
         start = remPtr_elem2D(n)
         call MPI_IRECV(recvbuf(start), e2D, MPI_DOUBLE_PRECISION, n, 2, MPI_COMM_FESOM_C, req(n), MPIerr)
      enddo
...
ELSE
   ...
   call MPI_SEND( arr2D, myDim_elem2D, MPI_DOUBLE_PRECISION, 0, 2, MPI_COMM_FESOM_C, MPIerr )
...
!      if (ierr/=0) print *,'allocate knall auf pe=0 in nod3D_real4',ierr
      ...
      do  n = 1, npes-1
         n3D = (remPtr_nod2D(n+1) - remPtr_nod2D(n))*nl1
         start = remPtr_nod2D(n)
         call MPI_IRECV(recvbuf(1,start), n3D, MPI_REAL, n, 2, MPI_COMM_FESOM_C, req(n), MPIerr)
      enddo
...
ELSE
   ...
   call MPI_SEND(sendbuf, myDim_nod2D*nl1, MPI_REAL, 0, 2, MPI_COMM_FESOM_C, MPIerr )
   deallocate(sendbuf)
...
IF ( mype == 0 ) THEN
   ...
   if (npes>1) then
      ...
      do  n = 1, npes-1
         n2D   = remPtr_nod2D(n+1) - remPtr_nod2D(n)
         start = remPtr_nod2D(n)
         call MPI_IRECV(recvbuf(start), n2D, MPI_REAL, n, 2, MPI_COMM_FESOM_C, req(n), MPIerr)
      enddo
...
ELSE
   ...
   call MPI_SEND(sendbuf, myDim_nod2D, MPI_REAL, 0, 2, MPI_COMM_FESOM_C, MPIerr )
...
IF ( mype == 0 ) THEN
   ...
   if (npes>1) then
      ...
      do  n = 1, npes-1
         e3D = (remPtr_elem2D(n+1) - remPtr_elem2D(n))*nl1
         start = remPtr_elem2D(n)
         call MPI_IRECV(recvbuf(1,start), e3D, MPI_REAL, n, 2, MPI_COMM_FESOM_C, req(n), MPIerr)
      enddo
...
ELSE
   ...
   call MPI_SEND(sendbuf, myDim_elem2D*nl1, MPI_REAL, 0, 2, MPI_COMM_FESOM_C, MPIerr )
   deallocate(sendbuf)
ENDIF
...
IF ( mype == 0 ) THEN
   ...
   if (npes>1) then
      ...
      do  n = 1, npes-1
         e2D   = remPtr_elem2D(n+1) - remPtr_elem2D(n)
         start = remPtr_elem2D(n)
         call MPI_IRECV(recvbuf(start), e2D, MPI_REAL, n, 2, MPI_COMM_FESOM_C, req(n), MPIerr)
      enddo
...
ELSE
   ...
   call MPI_SEND(sendbuf, myDim_elem2D, MPI_REAL, 0, 2, MPI_COMM_FESOM_C, MPIerr )
  ...
  IF ( mype == 0 ) THEN
     if (npes > 1) then
        ...
        do  n = 1, npes-1
            e2D   = remPtr_elem2D(n+1) - remPtr_elem2D(n)
            start = remPtr_elem2D(n)
            call MPI_IRECV(recvbuf(start), e2D, MPI_INTEGER, n, 2, MPI_COMM_FESOM_C, req(n), MPIerr)
        enddo      
        arr2D_global(myList_elem2D(1:myDim_elem2D)) = arr2D(1:myDim_elem2D)
        call MPI_WAITALL(npes-1, req, MPI_STATUSES_IGNORE, MPIerr)
        arr2D_global(remList_elem2D(1 : remPtr_elem2D(npes)-1)) &
...
IF ( mype == 0 ) THEN
   ...
   if (npes>1) then
      ...
      do  n = 1, npes-1
         n2D   = remPtr_nod2D(n+1) - remPtr_nod2D(n)
         start = remPtr_nod2D(n)
         call MPI_IRECV(recvbuf(start), n2D, MPI_INTEGER, n, 2, MPI_COMM_FESOM_C, req(n), MPIerr)
      enddo
...
ELSE
   ...
   call MPI_SEND( arr2D, myDim_nod2D, MPI_INTEGER, 0, 2, MPI_COMM_FESOM_C, MPIerr )
...
IF ( mype == 0 ) THEN
    ...
    ENDDO
ELSE
    CALL MPI_SEND( myDim_edge2D, 1, MPI_INTEGER, 0, 0, MPI_COMM_FESOM_C, MPIerr )
