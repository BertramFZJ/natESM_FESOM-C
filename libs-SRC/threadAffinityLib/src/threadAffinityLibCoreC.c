...
#ifdef _TALCC_USE_MPI_PLUGINS_
    char stdNodeNameString[MPI_MAX_PROCESSOR_NAME];
    for(i=0; i<MPI_MAX_PROCESSOR_NAME; i++) stdNodeNameString[i] = '\0';
#endif
...
#ifdef _TALCC_USE_MPI_PLUGINS_
        MPI_Get_processor_name(stdNodeNameString, &get);
        if(get > 0) sprintf(stdString, "[%s] ", stdNodeNameString);
#endif

    if( (mpiid <  0) && (ompid <  0) ) sprintf(stdString + strlen(stdString), "MASK: ");
    if( (mpiid >= 0) && (ompid <  0) ) sprintf(stdString + strlen(stdString), "MPIID %6d MASK: ", mpiid);
    if( (mpiid >= 0) && (ompid >= 0) ) sprintf(stdString + strlen(stdString), "MPIID %6d OMPTHREAD %3d MASK: ", mpiid, ompid);
